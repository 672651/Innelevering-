...
public class GPSComputer {
	...
	public double[] speeds() {
		...
		double[] time = new double[gpspoints.length-1];
	...
	public double kcal(double weight, int secs, double speed) {
		...
		// TODO - START
		if (speedmph > 0 && speedmph < 10) met = 4.0;
	...
	public void displayStatistics() {
		...
		String[][] statisticsWithDescriptions = {
		        {"Total Time", GPSUtils.formatTime(totalTime()), "hs:mm:ss"},
		        {"Total Distance", GPSUtils.formatDouble(totalDistance()/1000), "km"},
		        {"Total Elevation", GPSUtils.formatDouble(totalElevation()), "m"},
		        {"Max Speed", GPSUtils.formatDouble(maxSpeed()), "km/h"},
		        {"Average Speed", GPSUtils.formatDouble(averageSpeed()), "km/h"},
		    ...
		    for (int r = 0; r < statisticsWithDescriptions.length; r++) {
		      System.out.printf("%-" + stringWidth + "s", 
		          statisticsWithDescriptions[r][0]);
		      
		      System.out.println(":" + statisticsWithDescriptions[r][1] + " " +
